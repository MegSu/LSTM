...
def get_hidden_poetry(heading,total_length,temperature = 1.8):
    ...
    print()
    print('請輸入字詞：{0}'.format(heading))
    
    #print('----- temperature:', temperature)
    print('請輸入句數:', total_length)
    
    result = [] #設一個result串列去接收text_generated，最後要return
    text_generated = []
    heading_idx=len(heading)-1
    start_string = heading[-1]
    text_generated.append('[CLS]')
    text_generated.append(heading)
    ...
    while not is_finished:
        try:
            ...
            if text_generated[-2] == '[SEP]' and text_generated[-1] == '[PAD]':
                ...
                sys.stdout.flush()
            num_generate += 1
            if text_generated[-2] == ' ' and text_generated[-1] == ' ':  #計算text_generated有幾個斷行
                num_row += 1

            if num_row == total_length:  #如果等於使用者希望生成的行數就停止
                    is_finished = True
